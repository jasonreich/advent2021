...
pub fn parse_puzzle(file: &str) -> Puzzle {
  ...
  for line in read_file(file).unwrap() {
    if line.is_empty() {
      ...
      folds.push((captures[1].chars().nth(0).unwrap(), captures[2].parse().unwrap()))
    }
  }

  Puzzle { coords: coords, folds: folds }
}

pub fn part1(input: Puzzle) -> usize {
  let (fd, fp) = input.folds[0];
  input.coords.iter().map(|(x, y)| {
    match fd {
      'x' => {
        (if *x > fp { fp - (*x - fp) } else { *x }, *y)
      }
      'y' => {
        (*x, if *y > fp { fp - (*y - fp) } else { *y })
      }
      _ => unimplemented!()
    }
  }).unique().count()
