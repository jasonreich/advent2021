...
pub fn part1(input: impl Iterator<Item = Line>) -> usize {
    input
        .map(|(_, value)| {
            value
                .iter()
...
lazy_static! {
    static ref DIGITS: Vec<&'static str> = {
        vec![
...
fn solve((signals, value): Line) -> usize {
    let solutions: Vec<Vec<char>> = ('a'..='g')
        .permutations(7)
        .filter(|candidate| {
            signals
                .iter()
...
pub fn part2(input: impl Iterator<Item = Line>) -> usize {
    input.map(solve).sum()
}
